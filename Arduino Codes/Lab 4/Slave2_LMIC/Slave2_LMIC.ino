...
FlashStorage(error_storage, uint16_t);                 // Reserve a portion of flash memory, remove/comment when upload to the transmitter 
...
volatile bool newDataReceived = false; // Flag to indicate new data
...
void loop() {
  ...
  avgTemperature = Readtemp();
    ...
    if (newDataReceived) {
        ...
        // Now you can access LMIC.frame and LMIC.dataLen
        // SerialUSB.print("Received payload: ");
        // SerialUSB.write(LMIC.frame, LMIC.dataLen);
        SerialUSB.println();
        ...
        timeSinceLastPacket = millis(); //Timestamp this packet
          ...
          uint8_t authid = receivedPacket.authID;
              SerialUSB.print("Node ID: ");
              SerialUSB.println(nodeid);

          // if(nodeid == 1 && authid == 2){
                SerialUSB.println("Master asking for Payload....");
                float temperature = TempZero.readInternalTemperature();
                String temperatureString = String(temperature, 2);
                String AvhiTemperature = "Avhi Temp :" + temperatureString;
                // tx("First message going in...!", txdone_func);
                ...
                SerialUSB.print("Sending Packet to Master .......");
